...
export const getDeal = async (req, res) => {
	try {
		...
		// 	deal.farmer.toString() !== req.user.id
		// ) {
		// 	return res.status(403).json({
		// 		success: false,
		// 		message: "Unauthorized",
...
export const responseDeal = async (req, res) => {
	try {
		...
		const invalidstas = ["ACCEPTED", "REJECTED", "COMPLETED", "CANCELLED"];
		if (invalidstas.includes(deal.status)) {
			return res
				.status(400)
				.json({ success: true, message: "This deal is already closed" });
		}
		...
		// 	{ path: "crop", select: "name imageUrl price" },
		// ]);
